...
test("Points Mode issues participant access and personalizes private preview snapshots", () => {
  const store = createFixedStore();
  const created = store.createRoom({ audience: "group", mode: "competitive", cardsPerLevel: 4 });
  ...
  assert.equal(joinedTwo.room.mode, "competitive");
});

test("Points Mode rejects late joins and unauthorized participant actions", () => {
  const store = createFixedStore();
  const created = store.createRoom({ audience: "group", mode: "competitive", cardsPerLevel: 4 });
  store.joinRoom("PLAY5", "Jo");
...
test("Points Mode exposes host skip after a disconnected active player", async () => {
